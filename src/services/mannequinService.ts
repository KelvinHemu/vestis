/**
 * Mannequin Image Generation Service
 * Handles mannequin image generation API requests with proper error handling and authentication
 */
...
/**
 * MannequinService - Singleton service for mannequin image generation
 */
class MannequinService {
   ...
   */
  public static getInstance(): MannequinService {
    if (!MannequinService.instance) {
      MannequinService.instance = new MannequinService();
    }
    return MannequinService.instance;
  }

  /**
   * Generate a mannequin image
   * @param request - The generation request payload
   * @returns Promise with generation response
   */
  public async generateMannequin(
    request: GenerateFlatLayRequest
  ): Promise<GenerateFlatLayResponse> {
    try {
      // Log the request payload for debugging (with truncated images)
      console.log('🚀 Generating mannequin with request:', {
      ...
      // Add ?mdoli parameter to indicate request is from mannequin feature
      const response = await api.post('/v1/generate?mdoli', formattedRequest);
  ...
  /**
   * Check the status of a mannequin generation job
   * @param jobId - The job ID to check
   * @returns Promise with job status
   */
  public async getJobStatus(jobId: string): Promise<FlatLayJobStatus> {
    try {
      const response = await api.get(`/v1/mannequin/status/${jobId}`);
  ...
  /**
   * Get mannequin generation history
  ...
  public async getHistory(
    ...
    try {
      const response = await api.get(
        `/v1/mannequin/history?limit=${limit}&offset=${offset}`
      );
      ...
      console.error('Error fetching mannequin history:', error);
      throw error;
    }
  }

  /**
   * Get a specific mannequin generation by ID
   * @param id - The mannequin ID
   * @returns Promise with mannequin details
   */
  public async getMannequinById(id: string): Promise<FlatLayHistory> {
    try {
      const response = await api.get(`/v1/mannequin/${id}`);

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          errorData.message || `Failed to fetch mannequin: ${response.statusText}`
        );
      }
      ...
      console.error('Error fetching mannequin:', error);
      throw error;
    }
  }

  /**
   * Delete a mannequin from history
   * @param id - The mannequin ID to delete
   * @returns Promise with success status
   */
  public async deleteMannequin(id: string): Promise<{ success: boolean; message?: string }> {
    try {
      const response = await api.delete(`/v1/mannequin/${id}`);

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          errorData.message || `Failed to delete mannequin: ${response.statusText}`
        );
      }
...
export const mannequinService = MannequinService.getInstance();
