...
// ============================================================================
// ModelProfile Page - Dedicated model profile page (no sidebar)
// ============================================================================
...
export function ModelProfile() {
  ...
  const prevImage = () => {
    ...
    setCurrentImageIndex(prev => {
      ...
      return newIndex < 0 ? allImages.length + newIndex : newIndex; // Handle wrap around correctly? 
      // If length is 5. prev is 0. newIndex is -2. 5 + (-2) = 3.
      // If length is 4. prev is 0. newIndex is -2. 4 + (-2) = 2.
    });
  };
  
  // Ensure we have a valid index if images change (though they shouldn't for same model)
  useEffect(() => {
  ...
  // We want to display 2 images.
  // If we are at index i, we show i and i+1.
  // If i+1 is out of bounds, we wrap to 0? Or just show one?
  // Let's wrap to 0 for the second image if needed to always show 2.
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start relative justify-center">
          ...
          <div className="lg:w-48 flex-shrink-0 lg:sticky lg:top-32">
            <div className="grid grid-cols-2 gap-x-8 gap-y-4 lg:block lg:space-y-4 text-xs tracking-widest uppercase">
              ...
              {/* Placeholder stats to match the look */}
              <div>
          ...
          {/* Right Spacer to balance layout if needed, or just let images take space */}
